...
/* Sync Error Log Task */
const EVENT_RECEIVE_FOR_ERROR_LOG = rabbitConfig.EVENT_WORKER.EVENT_RECEIVE_FOR_ERROR_LOG;
const WORKER_RECEIVE_ERROR_LOG = rabbitConfig.EVENT_WORKER.WORKER_RECEIVE_ERROR_LOG;
const EVENT_PUBLISH_INSERT_POSTGRES = rabbitConfig.EVENT_WORKER.EVENT_PUBLISH_INSERT_POSTGRES;
const EVENT_RECEIVE_UNSUBSCRIPTION_USER_LOG = rabbitConfig.EVENT_WORKER.EVENT_RECEIVE_UNSUBSCRIPTION_USER_LOG;
const WORKER_RECEIVE_FOR_UNSUBSCRIPTION_USER_LOG = rabbitConfig.EVENT_WORKER.WORKER_RECEIVE_FOR_UNSUBSCRIPTION_USER_LOG
const EVENT_PUBLISH_UNSUBSCRIPTION_INSERT_POSTGRES = rabbitConfig.EVENT_WORKER.EVENT_PUBLISH_UNSUBSCRIPTION_INSERT_POSTGRES
const RawDataProcessWorker = new Rabbit.default({
    ...
    queues: [{
        ...
        name: WORKER_RECEIVE_ERROR_LOG,
        exchange: Rabbit.JOB_EXCHANGE.name,
        concurrency: 5000
    }, {
        name:WORKER_RECEIVE_FOR_UNSUBSCRIPTION_USER_LOG,
        exchange: Rabbit.JOB_EXCHANGE.name,
        concurrency: 5000
    }]
});
...
let count = 0;
let dataPostges = {};
/*Listen UnSubscription User Log message*/
RawDataProcessWorker.listen(EVENT_RECEIVE_UNSUBSCRIPTION_USER_LOG,WORKER_RECEIVE_FOR_UNSUBSCRIPTION_USER_LOG,function(item){
    if(item){
        console.log('gia tri nhan duoc tu rawData:',item)
        RawDataProcessWorker.publish(EVENT_PUBLISH_UNSUBSCRIPTION_INSERT_POSTGRES,item,Rabbit.PRIORITY.normal)
    }
})
    ...
    if (rawData) {
        generateData(rawData)
            .then(function (userGenerated) {
                // console.log('userGenerated', userGenerated)
...
/* Listen Sync Error Log Message */
RawDataProcessWorker.listen(EVENT_RECEIVE_FOR_ERROR_LOG, WORKER_RECEIVE_ERROR_LOG, function (records) {
    let SyncErrorLogData = []
    if (records) {
        records.map(record => {
            let recordConverted = {
                userID: record.userId,
                platform: record.platform,
                statusCode: record.errorCode || record.statusCode,
                logDate: record.errorDate || record.logAt,
                url: record.url,
                createAt: new Date()
                
            }
            SyncErrorLogData.push(recordConverted);
        })
    }

    //if (SyncErrorLogData.length > 0) RawDataProcessWorker.publish(EVENT_PUBLISH_INSERT_POSTGRES, SyncErrorLogData, Rabbit.PRIORITY.normal)
    if(SyncErrorLogData.length >0) {
        SyncErrorLogData.forEach(function(item, index, array){
            setTimeout(() =>{
                console.log('stt',index)
                RawDataProcessWorker.publish(EVENT_PUBLISH_INSERT_POSTGRES, item,Rabbit.PRIORITY.normal)
            },index*100)
        })
...
let publishForSaleInsert = function (record, callback) {
    generateDataSale(record)
        .then(function (data) {
            console.log('generateDataSale', data)
