...
const EVENT_PUBLISH_PURCHASE_REQUEST_SYNC_ERROR_LOG = rabbitConfig.EVENT_WORKER.EVENT_PUBLISH_PURCHASE_REQUEST_SYNC_ERROR_LOG;
const EVENT_PUBLISH_UNSUBSCRIPTION_USER_LOG = rabbitConfig.EVENT_WORKER.EVENT_PUBLISH_UNSUBSCRIPTION_USER_LOG
...
require(path.join(__dirname, '../../model/sync_error_log'));
require(path.join(__dirname, '../../model/purchase_request_log'))
//dung cho task ngay 2/1/2018
require(path.join(__dirname,'../../model/unsubcription_user_log'))
// Connect to MongoDB
...
const SyncErrorLog = mongoose.model('SyncErrorLog');
const PurchaseRequestLog = mongoose.model('PurchaseRequestLog')
const UnSubscriptionUserLog = mongoose.model('unsubscriptionuser')

const SELECT_USER_COLLECTION = '_id createdDate lastSync tags purchased premium_at isDeactivated rwExpire tags';
// const SELECT_USER_COLLECTION = '_id';
const SELECT_DEVICE_COLLECTION = '_id owner';

const SELECT_ERROR_LOG_COLLECTION = 'userId platform errorCode errorDate url';
// const SELECT_ERROR_LOG_COLLECTION = '_id';
const SELECT_PURCHASE_LOG_COLLECTION = 'userId platform statusCode logAt url';

const SELECT_UNSUBSCRIPTION_USER_LOG='userId platform bill_id product_id create_at'
...
// Get UnSubscriptionUserlogs from collection 'unsubscriptionuser' in mongodb
let getListUnSubUserLogs = function(callback){
    let startDay = "2017-04-04T17:00:00.000Z";
    let endDay = "2017-04-10T17:15:05.000Z";
    let condition  ={
        create_at: {
            $gte: startDay,
            $lt: endDay
    ...
    UnSubscriptionUserLog.find(condition)            
            .select(SELECT_UNSUBSCRIPTION_USER_LOG)
            .lean(true)
            .exec(callback)            
}

function getSynErrorLogMain() {
    let startDay = "2017-08-01T17:00:00.000Z";
    let endDay = "2017-08-30T17:00:00.000Z";
    let syncErrorLog, purchaseRequestLog;

    let getDataLogs = cb => {
        return new Promise((resolve, reject) => {
            cb(startDay, endDay, (err, result) => {
                resolve(result);
            });
        });
    };

    getDataLogs(getSyncErrorLog)
        .then(data => {
            syncErrorLog = data;
            return getDataLogs(getPurchaseRequestLog);
        })
        .then(data => {
            purchaseRequestLog = data
            let results = syncErrorLog.concat(purchaseRequestLog)

            RawDataPublisher.publish(EVENT_PUBLISH_PURCHASE_REQUEST_SYNC_ERROR_LOG, results, Rabbit.PRIORITY.normal);
        });
};

let getSyncErrorLog = (startDay, endDay, callback) => {
    let condition = {
        errorDate: {
            $gte: startDay,
            $lt: endDay
    ...
    SyncErrorLog.find(condition)
        .select(SELECT_ERROR_LOG_COLLECTION)
        .lean(true)
        .exec(callback)
}

let getPurchaseRequestLog = (startDay, endDay, callback) => {
    let condition = {
        logAt: {
            $gte: startDay,
            $lt: endDay
    ...
    PurchaseRequestLog.find(condition)
        .select(SELECT_PURCHASE_LOG_COLLECTION)
        .lean(true)
        .exec(callback)
}
...
let getListUser = function (callback) {
    ...
    let startDay = "2014-05-01T17:00:00.000Z";
    let endDay = "2014-05-31T17:52:43.914Z";
...
*/
function getLists(){
    getListUnSubUserLogs( function(error, users) {
        if(error) {
            return;
        }
        users.forEach( function(user,index,collection) {
            setTimeout(function(){
                ...
                RawDataPublisher.publish(EVENT_PUBLISH_UNSUBSCRIPTION_USER_LOG,user,Rabbit.PRIORITY.normal)
            },index*150)
        })
    })
}
