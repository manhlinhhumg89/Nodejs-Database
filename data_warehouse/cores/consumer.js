...
const EVENT_RECEIVE_INSERT_POSTGRES = rabbitConfig.EVENT_WORKER.EVENT_RECEIVE_INSERT_POSTGRES;
const WORKER_RECEIVE_POSTGRES_LOG = rabbitConfig.EVENT_WORKER.WORKER_RECEIVE_POSTGRES_LOG
const WORKER_RECEIVE_UNSUB_POSTGRES_LOG = rabbitConfig.EVENT_WORKER.WORKER_RECEIVE_UNSUB_POSTGRES_LOG
const EVENT_RECEIVE_UNSUBSCRIPTION_INSERT_POSTGRES = rabbitConfig.EVENT_WORKER.EVENT_RECEIVE_UNSUBSCRIPTION_INSERT_POSTGRES
...
const PostgresWorker = new Rabbit.default({
    ...
    queues: [{
        ...
        name: WORKER_RECEIVE_POSTGRES_LOG,
        exchange: Rabbit.JOB_EXCHANGE.name,
        concurrency: 5000
    },{
        name: WORKER_RECEIVE_UNSUB_POSTGRES_LOG,
        exchange: Rabbit.JOB_EXCHANGE.name,
        concurrency: 5000
    }]
});
...
const NEW_TABLE_NAME ="unsubscription_user_log"
...
// insert vao postgres task moi

PostgresWorker.listen(EVENT_RECEIVE_UNSUBSCRIPTION_INSERT_POSTGRES,WORKER_RECEIVE_UNSUB_POSTGRES_LOG, function(record,next){
    console.log('gia tri nhan duoc tu dataProcessor:',record)
    //InsertLogToPostgres(record,next)
    
})
PostgresWorker.listen(EVENT_RECEIVE_INSERT_POSTGRES, WORKER_RECEIVE_POSTGRES_LOG, function (records, next) {
    //console.log('gia tri nhan tu DataProcess:',records);
    insertPgDB(records,next)
});
let InsertLogToPostgres = function(item,callback) {
    InsertUnSubscriptionLog(item,NEW_TABLE_NAME)
        .then(function(res) {
            callback()
        })
        .catch(function(err){
...
function InsertUnSubscriptionLog(data,tablename){
    return new Promise( function(resolve,reject) {
        ...
        let rawQuery = {
            query: "INSERT INTO " + tablename + "(userid, platform, billid, productid,createdat)" +"VALUES($1,$2,$3,$4,$5) RETURNING userid",
            data:[
                userId,platform,billId,productId,create_at
            ]
        }

        POSTGRESDB.tx((db)=>{
            db.query(rawQuery.query, rawQuery.data)
        })
            .then(res =>{
                resolve()
            })
...
// let insertPgDB = function(item, callback){
//     insertQuery(item, TABLE_NAME)
//         .then(function(res) {
//             callback()
//         })
//         .catch( function(err){
...
//         let userId = data.userID;
//         let platForm = data.platform;
//         let statuscode1 = data.statusCode;
//         let url1 = data.url;
//         let dc = data.createAt;
//         let logdate = new Date(data.logDate);
//         let rawQuery = {
//             query: "INSERT INTO " + tablename + "(_us,_plform,_stcode,_url,_log,_createdate)"+ "VALUES ($1,$2,$3,$4,$5,$6) RETURNING _us",
//             data:[
//                 userId,platForm,statuscode1,url1,logdate,dc
//             ]
//         }
//         POSTGRESDB.tx((db) =>{
//             db.query(rawQuery.query, rawQuery.data)
